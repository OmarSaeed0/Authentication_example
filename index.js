...
// sample user data
const users = [
  { id: 1, username: 'johndoe', password: 'samplePassword',ID_NUMBER:"11" },
];
...
app.post('/login', (req, res) => {
  ...
  const user = users.find(u => u.username === username && u.password === password && u.ID_NUMBER ===ID_NUMBER);

  if (user) {
    // Generate a JWT token
    const token = jwt.sign({ id: user.id, username: user.username,ID_NUMBER: user.ID_NUMBER}, secretKey);
